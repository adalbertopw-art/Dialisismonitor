...
export function NursingNotesMiner({ patient }: any) {
  ...
  return (
    <Card className="bg-background border-border shadow-xl mt-6 relative overflow-hidden group">
      ...
      <CardHeader className="py-4 px-6 border-b border-border bg-gradient-to-r from-indigo-500/10 to-transparent">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {!analyzed && !analyzing && (
            <button
              ...
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-foreground text-[10px] font-bold uppercase tracking-widest rounded transition-colors flex items-center gap-2"
            >
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-white/5">
            ...
            <div className="p-6">
               <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-4 flex items-center gap-2">
                 <FileText size={14} className="text-muted-foreground" /> Notas Libres de Enfermería
               </h4>
               <div className="space-y-4">
                 {rawNotes.map((note, idx) => (
                    <div key={idx} className="bg-card p-3 rounded-lg border border-border">
                       <div className="flex items-center justify-between mb-2 pb-2 border-b border-border text-[9px] font-bold uppercase tracking-widest text-muted-foreground">
                         <span>{note.date}</span>
                         <span>Enf. {note.nurse}</span>
                       </div>
                       <p className={`text-[11px] leading-relaxed ${analyzed ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
                          {analyzed ? (
            ...
            <div className="p-6 bg-gradient-to-b from-transparent to-indigo-500/5">
               ...
               {analyzing ? (
                    ...
                    <div className="bg-card border border-border rounded-lg p-4">
                       ...
                       <div className="space-y-3">
                         <div className="flex items-center justify-between bg-muted/50 p-2 rounded">
                            <span className="text-[11px] font-medium text-foreground">Hipotensión</span>
                            <Badge variant="outline" className="text-[9px] bg-rose-500/10 text-rose-400 border-rose-500/20">Crítico</Badge>
                         </div>
                         <div className="flex items-center justify-between bg-muted/50 p-2 rounded">
                            <span className="text-[11px] font-medium text-foreground">Calambres Musculares</span>
                            <Badge variant="outline" className="text-[9px] bg-amber-500/10 text-amber-400 border-amber-500/20">Moderado</Badge>
                         </div>
                         <div className="flex items-center justify-between bg-muted/50 p-2 rounded">
                            <span className="text-[11px] font-medium text-foreground">Náuseas</span>
                            <Badge variant="outline" className="text-[9px] bg-amber-500/10 text-amber-400 border-amber-500/20">Moderado</Badge>
                         </div>
                         <div className="flex items-center justify-between bg-muted/50 p-2 rounded">
                            <span className="text-[11px] font-medium text-foreground">Mareos</span>
                            <Badge variant="outline" className="text-[9px] bg-emerald-500/10 text-emerald-400 border-emerald-500/20">Leve</Badge>
                         </div>
                         <div className="flex items-center justify-between bg-muted/50 p-2 rounded">
                            <span className="text-[11px] font-medium text-foreground">Fatiga Post-Diálisis</span>
                            <Badge variant="outline" className="text-[9px] bg-emerald-500/10 text-emerald-400 border-emerald-500/20">Leve</Badge>
                         </div>
                       </div>
                    </div>
                 ...
                 <div className="flex flex-col items-center justify-center py-16 opacity-30">
                   <BrainCircuit size={48} className="mb-4 text-muted-foreground" />
